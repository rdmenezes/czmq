...
s_tickless_timer (zloop_t *self)
{
	...
	tickless = zclock_time () + 1000 * 3600;
	...
	timeout = (long) (tickless - zclock_time ());
	if (timeout < 0)
		timeout = 0;
	if (self->verbose)
...
zloop_timer (zloop_t *self, size_t delay, size_t times, zloop_fn handler, void *arg)
{
	...
	timer = s_timer_new (delay, times, handler, arg);
	if (!timer)
		return -1;
...
zloop_start (zloop_t *self)
{
	...
	while (!zctx_interrupted) {
		...
		for (item_nbr = 0; (int)item_nbr < self->poll_size; item_nbr++) {
			poller = &self->pollact [item_nbr];
...
zloop_test (Bool verbose)
{
	...
	zloop_timer (loop, 10, 1, s_timer_event, output);	/* 0 After 10 msecs, send a ping message to output */
	poll_input.socket = input;							/* 0 When we get the ping message, end the reactor */
	poll_input.fd = 0;									/* 1                                               */
	poll_input.events = ZMQ_POLLIN;						/* 2                                               */
